...
curl -s -X POST -k --key $GATEWAY_KEY --cert $GATEWAY_CERT -H "Content-Type: application/json" -d "$(envsubst < ./requests/pool_request_1.json)" https://localhost:5000/amm/pool | jq

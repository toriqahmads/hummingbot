...
export async function addWallet(
  ...
  if (req.chain === 'ethereum') {
    ...
    const cronos = Cronos.getInstance(req.network);
    address = cronos.getWalletFromPrivateKey(req.privateKey).address;
    encryptedPrivateKey = await cronos.encrypt(req.privateKey, passphrase);
  } else {

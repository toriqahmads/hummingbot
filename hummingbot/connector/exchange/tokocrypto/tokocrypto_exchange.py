...
from hummingbot.connector.exchange.tokocrypto.tokocrypto_api_order_book_data_source import (
    TokocryptoAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.tokocrypto.tokocrypto_api_user_stream_data_source import (
    TokocryptoAPIUserStreamDataSource,
)
from hummingbot.connector.exchange.tokocrypto.tokocrypto_auth import TokocryptoAuth
...
class TokocryptoExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self.api_key = tokocrypto_api_key
        self.secret_key = tokocrypto_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_tokocrypto_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def tokocrypto_order_type(order_type: OrderType) -> str:
        if order_type.name.upper() == "LIMIT":
            return "1"
        if order_type.name.upper() == "LIMIT_MAKER":
            return "1"
        if order_type.name.upper() == "MARKET":
            return "2"
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return TokocryptoAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return TokocryptoAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        symbol = self.tokocrypto_exchange_symbol_associated_to_pair(trading_pair=trading_pair)
        ...
        order_result = await self._api_post(
            path_url=CONSTANTS.ORDER_BUY_URL,
            data=self.authenticator.add_auth_to_params(api_params),
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        cancel_result = await self._api_post(
            ...
            params=self.authenticator.add_auth_to_params(api_params),
            headers=self.authenticator.header_for_authentication(),
            is_auth_required=False,
            use_private=True)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(tokocrypto_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_tokocrypto_timestamp * 1e3)
            self._last_trades_poll_tokocrypto_timestamp = self._time_synchronizer.time()
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        if order.exchange_order_id is not None:
            ...
            trading_pair = self.tokocrypto_exchange_symbol_associated_to_pair(trading_pair=order.trading_pair)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(tokocrypto_utils.is_exchange_information_valid, exchange_info["symbols"]):
    ...
    async def _api_request(self,
        ...
        if use_private or is_auth_required:
